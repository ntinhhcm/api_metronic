		...
		var Plugin = {
			...
			initHeight: function () {
				if (options.layout.height && options.layout.scroll) {
					var theadHeight = 0;
					var tfootHeight = 0;
					if ( ! options.extraHeadTitle) {
						theadHeight = $(datatable.tableHead).find('.m-datatable__row').height();
						tfootHeight = $(datatable.tableFoot).find('.m-datatable__row').height();
					} else {
						var __theads = $(datatable.tableHead).find('.m-datatable__row');
						__theads.each(function(index, row) {
							theadHeight += $(this).height();
						});
						var __tfoots = $(datatable.tableFoot).find('.m-datatable__row');
						__tfoots.each(function(index, row) {
							tfootHeight += $(this).height();
						}); 
					}
			...
			setHeadTitle: function (tablePart) {
				...
				// Add extra header to thead
				if (options.extraHeadTitle) {
					$(tablePart).prepend(options.extraHeadTitle.template);
				}
